...
namespace GameFrameworkDesign.Example.ShootGame { 
	...
	public class GunConfigItem { 
		...
		public float ReloadSceonds { get; set; }
		public string Description { get; set; }

        public GunConfigItem(string gunName, int bulletMaxCount, float attack, float frequency, float shootDistance, bool needBullet, float reloadSceonds, string description)
        {
            GunName = gunName;
	...
	public class GunConfigModel : AbstractModel, IGunConfigModel {
        ...
        private Dictionary<string, GunConfigItem> mGunConfigItemDict = new Dictionary<string, GunConfigItem>() {
            //                          名称 弹夹容量 攻击力 频率 射程/范围 限制 换弹时间 描述
        ...
        {
            return mGunConfigItemDict[gunName];
        }
    }
}
